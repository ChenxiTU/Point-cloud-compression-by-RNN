...
#
image = image.astype(np.float32) / 255/100
image = np.pad(image,((0,0),(0,4)),'constant')
...
# end = time.time()
# print('P1 cost: {}'.format((end - start)))
...
decoder_h_4 = (Variable(
    torch.zeros(batch_size, 128, height // 2, width // 2), volatile=True),
               Variable(
                   ...
                   volatile=True))
# end = time.time()
# print('P2 cost: {}'.format((end - start)))
...
codes = []
res = image - 5200/255/100
# end = time.time()
# print('P3 cost: {}'.format((end - start)))
...
# end = time.time()
# print('All iterations cost: {}'.format((end - start)))
